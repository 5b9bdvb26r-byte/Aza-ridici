...
export default function EmployeesManagement() {
  ...
  // Formulář pro nového zaměstnance
  ...
  const handleAddDriver = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Chyba při vytváření zaměstnance');
      }

      await fetchDrivers();
      setShowAddForm(false);
      setNewDriver({ name: '', color: '#3B82F6', password: '', role: 'DRIVER' });
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Neznámá chyba');
    } finally {
  ...
  const handleUpdateDriver = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Chyba při aktualizaci zaměstnance');
      }
  ...
  const handleDeleteDriver = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Chyba při mazání zaměstnance');
      }
    ...
    <div>
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Přidávejte, upravujte a mazejte zaměstnance
          </p>
        </div>
        <button
      ...
      <div className="card">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">
          Zaměstnanci ({drivers.length})
        </h2>

        {drivers.length === 0 ? (
          <p className="text-gray-500 text-center py-8">
            Žádní zaměstnanci v systému
          </p>
        ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-gray-900 text-lg">
                      ...
                      <span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${
                        ...
                        {driver.role === 'WAREHOUSE' ? 'Skladník' : 'Řidič'}
                      </span>
                    </div>
            ...
            <form onSubmit={handleAddDriver} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Jméno zaměstnance (slouží i pro přihlášení)
                </label>
                <input
                  type="text"
                  required
              ...
              <div>
                ...
                >
                  ...
                  <option value="WAREHOUSE">Skladník</option>
                </select>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white rounded-t-xl sm:rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-6">
              Opravdu chcete smazat zaměstnance <strong>{deletingDriver.name}</strong>?
              Tato akce je nevratná a smaže všechna data spojená s tímto zaměstnancem.
            </p>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white rounded-t-xl sm:rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-2">
              Resetovat hodnocení zaměstnance <strong>{resetRatingDriver.name}</strong>?
            </p>
